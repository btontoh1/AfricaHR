...
export function JournalEntryForm({ tenantId }: { tenantId: string }) {
  const router = useRouter();
  const createEntry = useCreateJournalEntry(tenantId);
    ...
    try {
      await createEntry.mutateAsync({
        ...
        lines: values.lines.map((line) => ({
          // Validated by AccountPicker's own options (always one of the
          // fixed codes GET /finance/accounts returns) and re-validated by
          // the backend regardless - this cast just bridges the plain
          // string the <Select> works with to the DTO's literal union.
          accountCode: line.accountCode as ManualJournalEntryLineInput['accountCode'],
      ...
      toast.error(getApiErrorMessage(error, 'Failed to create journal entry'));
    }
  }
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} noValidate className="space-y-6">
        ...
        <Button type="submit" disabled={form.formState.isSubmitting || !balance.isBalanced}>
          {form.formState.isSubmitting ? 'Saving…' : 'Create journal entry'}
        </Button>
        {!balance.isBalanced && (
