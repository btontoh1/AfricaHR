...
describe('RecruitmentApplicationStageChangedNotificationListener', () => {
  let listener: RecruitmentApplicationStageChangedNotificationListener;
  ...
  beforeEach(() => {
    ...
    listener = new RecruitmentApplicationStageChangedNotificationListener(notifications, users);
  });

  it('sends an IN_APP notification to the hiring manager', async () => {
    await listener.handleApplicationStageChanged({ ...basePayload, hiringManagerUserId: 'mgr-user-1' });

    expect(notifications.send).toHaveBeenCalledWith('tenant-1', {
      userId: 'mgr-user-1',
      channel: 'IN_APP',
      subject: "Kwame Asante's application moved to HIRED",
  ...
  it('does not notify the actor even when they are the hiring manager', async () => {
    await listener.handleApplicationStageChanged({ ...basePayload, hiringManagerUserId: 'hr-1' });
  ...
  it('still notifies tenant admins/HR managers when there is no hiring manager', async () => {
    ...
    await listener.handleApplicationStageChanged({ ...basePayload, hiringManagerUserId: null });
  ...
  it('notifies no one and does not throw when there is no hiring manager and no admins', async () => {
    await expect(
      listener.handleApplicationStageChanged({ ...basePayload, hiringManagerUserId: null }),
  ...
  it('swallows a dispatch failure for one recipient without skipping the others', async () => {
    ...
    await expect(
      listener.handleApplicationStageChanged({ ...basePayload, hiringManagerUserId: 'mgr-user-1' }),
    ).resolves.toBeUndefined();
