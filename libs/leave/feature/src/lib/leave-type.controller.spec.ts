...
describe('LeaveTypeController', () => {
  ...
  let service: jest.Mocked<LeaveTypeService>;
  ...
  beforeEach(() => {
    service = {
      ...
      list: jest.fn(),
      update: jest.fn(),
    } as unknown as jest.Mocked<LeaveTypeService>;

    controller = new LeaveTypeController(service);
  });

  it('creates within the route tenant when the actor matches', () => {
    const dto = { name: 'Annual Leave', code: 'ANNUAL', defaultEntitlementDays: 15 } as never;
  ...
  it('delegates update with tenant, id, dto, and actor', () => {
    ...
    controller.update('tenant-1', 'lt-1', dto, hrManager);

    expect(service.update).toHaveBeenCalledWith('tenant-1', 'lt-1', dto, 'hr-1');
  });
});
